...
export function createDataSlice<D, E, P extends unknown[] = unknown[]>({
  ...
  const call = async (...params: P) => {
    ...
    const response = isPromise(initialCall)
      ? initialCall
      : initialCall(getState().data);
...
function createGETSlice<Data, E>(
  initialState: State<Data, E>,
  resource: string,
  name: string
) {
  return createSlice(
...
function createPOSTSlice<Data, E>(
  initialState: State<Data, E>,
  resource: string,
  name: string,
  optimisticUpdate?: OptimisticUpdate<Data>
) {
      ...
      [`${name}/pending`]: (state: State<Data, E>, payload: Data) => {
        ...
        return {
          ...
          data: optimisticUpdate
            ? getOptimisticData<Data>(optimisticUpdate, state.data, payload)
            : state.data,
          loading: true,
          updating: state.data !== null,
        };
...
function createPUTSlice<Data, E>(
  initialState: State<Data, E>,
  resource: string,
  name: string,
  optimisticUpdate?: OptimisticUpdate<Data>
...
function createDELETESlice<Data, E>(
  initialState: State<Data, E>,
  resource: string,
  name: string,
  optimisticUpdate?: OptimisticUpdate<Data>
  ...
  return createSlice(
    {
      [`${name}/pending`]: (state: State<Data, E>) => {
        ...
        return {
          ...state,
          data: optimisticUpdate
            ? getOptimisticData<Data>(optimisticUpdate, state.data)
            : state.data,
          loading: true,
          updating: state.data !== null,
        };
